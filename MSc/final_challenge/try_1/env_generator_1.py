...
# def generate_single_obstacle_env(dim, filename):
#     w, h = dim
#     obstacle = (random.randint(0, w - 1), random.randint(0, h - 1))
#     with open(filename, "w") as f:
#         yaml.dump({"map": {"dimensions": dim, "obstacles": [list(obstacle)]}}, f)

# def generate_channel_env(dim, filename, orientation="vertical"):
#     w, h = dim
#     obstacles = []
...
# def generate_cross_obstacles(dim, filename):
#     w, h = dim
#     obstacles = [(x, h//2) for x in range(w) if x != w//2] + [(w//2, y) for y in range(h) if y != h//2]
#     with open(filename, "w") as f:
#         yaml.dump({"map": {"dimensions": dim, "obstacles": [list(o) for o in obstacles]}}, f)
