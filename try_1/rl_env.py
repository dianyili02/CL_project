...
#         self.collisions = [False] * self.num_agents  # reset every step
...
#             else:
#                 proposed_positions.append((r, c))  # 非法动作 -> 原地
...
class PRIMALEnvironment:
    ...
    def reset(self):
        ...
        for i in range(self.num_agents):
            if self.agent_positions[i] == self.agent_goals[i]:
                self.dones[i] = True
                print(f"[DEBUG RESET] Agent {i} already at goal {self.agent_goals[i]}")
    ...
    def step(self, actions):
        ...
        for i in range(self.num_agents):
            ...
            if new_pos == goal_pos and not self.dones[i]:
                reward = 1.0
                new_dones[i] = True
                print(f"[DEBUG] Agent {i} reached goal at {new_pos}")
            else:
        ...
        self.agent_positions = final_positions
        self.dones = new_dones
        # print(f"[DEBUG STEP] dones = {new_dones}")
        observations = [self.get_observation(i) for i in range(self.num_agents)]
        return observations, rewards, self.dones.copy(), {}
